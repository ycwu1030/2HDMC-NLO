...
bool THDM::set_param_gen(double lambda1, double lambda2, double lambda3,
    ...
    // Every time user set parameters, the physical parameters should be updated.
    Updated = false;
...
void THDM::get_coupling_hdd_NLO(int h,int f1,int f2,complex <double> &cs, complex <double> &cp) {
  complex <double> I(0.0,1.0);
  ...
  if ((f1<=3)&&(f1>=1)&&(f2<=3)&&(f2>=1)) {
    if (f1==f2) {
      double mms  = sm.get_dmass_MSbar(f1);
      double mp   = sm.get_dmass_pole(f1);
      double mmh  = get_hmass(h);
      CFunc_NLO FCoup;
      int status = GetNLOFuncSFF(h,DOID+f1,DOID+f2,yukawas_type,FCoup);
      if (status == NOTIMPLEMENT)
      {
        get_coupling_hdd(h,f1,f2,cs,cp);
        return;
      }
      if (!Updated) store_param_phys();
      x = FCoup(L_MHL2,L_MHH2,L_MHA2,L_MHP2,L_M2,L_beta,L_alp,mmh*mmh,mp*mp,mp*mp,sm);
      // Starting scale for MSbar mass evolution
      double Qinit = mms;
      if (f1==2) {
       // Special case of strange mass where scale is not ms(ms) but ms(Q_ms=2 GeV)
...
void THDM::get_coupling_huu_NLO(int h,int f1,int f2,complex <double> &cs, complex <double> &cp) {
  complex <double> I(0.0,1.0);
  ...
  if ((f1<=3)&&(f1>=1)&&(f2<=3)&&(f2>=1)) {
    if (f1==f2) {
      ...
      double mmh  = get_hmass(h);
      CFunc_NLO FCoup;
      int status = GetNLOFuncSFF(h,UPID+f1,UPID+f2,yukawas_type,FCoup);
      if (status == NOTIMPLEMENT)
      {
          get_coupling_huu(h,f1,f2,cs,cp);
          return;
      }
      if (!Updated) store_param_phys();
      x = FCoup(L_MHL2,L_MHH2,L_MHA2,L_MHP2,L_M2,L_beta,L_alp,mmh*mmh,mp*mp,mp*mp,sm);
...
void THDM::get_coupling_hll_NLO(int h,int f1,int f2,complex <double> &cs, complex <double> &cp) {
  complex <double> I(0.0,1.0);
  ...
  if ((f1<=3)&&(f1>=1)&&(f2<=3)&&(f2>=1)) {
    double mp = sm.get_lmass_pole(f1);
    double mmh = get_hmass(h);
    CFunc_NLO FCoup;
    int status = GetNLOFuncSFF(h,LEID+f1,LEID+f2,yukawas_type,FCoup);
    if (status == NOTIMPLEMENT)
    {
        get_coupling_hll(h,f1,f2,cs,cp);
        return;
    }
    if (!Updated) store_param_phys();
    x = FCoup(L_MHL2,L_MHH2,L_MHA2,L_MHP2,L_M2,L_beta,L_alp,mmh*mmh,mp*mp,mp*mp,sm);
...
void THDM::get_coupling_hdu_NLO(int h,int d,int u,complex <double> &cs, complex <double> &cp) {
...
void THDM::get_coupling_hln_NLO(int h,int l,int n,complex <double> &cs, complex <double> &cp) {
// Not yet implement, using LO results instead
  complex <double> I(0.0,1.0);
...
//   if (v==2)
//   {
//     if(((h1==1)&&(h2==3))||((h1==3)&&(h2==2))){
//       double L_m_h,L_m_H,L_m_A,L_m_Hp,L_sba,L_lambda6,L_lambda7,L_m12_2,L_tan_beta; 
//       get_param_phys(L_m_h,L_m_H, L_m_A, L_m_Hp,
//                       L_sba, L_lambda6, L_lambda7,
//                       L_m12_2, L_tan_beta);
//       double L_beta = atan(L_tan_beta);
//       double L_M2 = L_m12_2/(sin(L_beta)*cos(L_beta));
//       double L_alp = L_beta - asin(L_sba);
//       double L_m_Z = sm.get_vmass(2);
//       if (yukawas_type==1)
//       {
//         c = TypeI::CAhZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_A,2), pow(L_m_h,2), pow(L_m_Z,2),sm);
//       }
//       else if (yukawas_type==2)
//       {
//         c = TypeII::CAhZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_A,2), pow(L_m_h,2), pow(L_m_Z,2),sm);
//       }
//       else if (yukawas_type==3)
//       {
//         c = TypeLS::CAhZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_A,2), pow(L_m_h,2), pow(L_m_Z,2),sm);
//       }
//       else if (yukawas_type==4)
//       {
//         c = TypeFL::CAhZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_A,2), pow(L_m_h,2), pow(L_m_Z,2),sm);
//       }
//       else
//       {
//         c = TypeII::CAhZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_A,2), pow(L_m_h,2), pow(L_m_Z,2),sm);
...
void THDM::get_coupling_vhh_NLO(int v,int h1,int h2,complex <double> &c) {
  // Convention of hep-ph/0602242
  complex <double> I(0.0,1.0);
  ...
  // gamma
  // For gamma-Hp-Hm, still using LO results, since we only care about the decay channels.
  if (v==1) {
    if ((h1==4)&&(h2==4)) { c=-I*e; }
    return;
  }
  ...
  CFunc_NLO FCoup;
  int status = GetNLOFuncSSV(h1,h2,v,yukawas_type,FCoup);
  if (status == NOTIMPLEMENT)
  {
      get_coupling_vhh(v,h1,h2,c);
      return;
  }
  if (!Updated) store_param_phys();
  double mh1 = get_hmass(h1);
  double mh2 = get_hmass(h2);
  double mv = sm.get_vmass(v);
  c = FCoup(L_MHL2,L_MHH2,L_MHA2,L_MHP2,L_M2,L_beta,L_alp,mh1*mh1,mh2*mh2,mv*mv,sm);
}
...
//   double L_alp = L_beta - asin(L_sba);
//   double L_m_Z = sm.get_vmass(2);
//   double L_m_W = sm.get_vmass(3);
//   if ((v1==2)&&(v2==2)&&(h==2)) {
//     if (yukawas_type==1)
//     {
//       c = TypeI::CHHZZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_Z,2), pow(L_m_Z,2),sm);
//     }
//     else if (yukawas_type==2)
//     {
//       c = TypeII::CHHZZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_Z,2), pow(L_m_Z,2),sm);
//     }
//     else if (yukawas_type==3)
//     {
//       c = TypeLS::CHHZZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_Z,2), pow(L_m_Z,2),sm);
//     }
//     else if (yukawas_type==4)
//     {
//       c = TypeFL::CHHZZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_Z,2), pow(L_m_Z,2),sm);
//     }
//     else
//     {
//       c = TypeII::CHHZZ(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_Z,2), pow(L_m_Z,2),sm);
...
//     if (yukawas_type==1)
//     {
//       c = TypeI::CHHWW(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_W,2), pow(L_m_W,2),sm);
//     }
//     else if (yukawas_type==2)
//     {
//       c = TypeII::CHHWW(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_W,2), pow(L_m_W,2),sm);
//     }
//     else if (yukawas_type==3)
//     {
//       c = TypeLS::CHHWW(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_W,2), pow(L_m_W,2),sm);
//     }
//     else if (yukawas_type==4)
//     {
//       c = TypeFL::CHHWW(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_W,2), pow(L_m_W,2),sm);
//     }
//     else
//     {
//       c = TypeII::CHHWW(pow(L_m_h,2), pow(L_m_H,2), pow(L_m_A,2), pow(L_m_Hp,2), L_M2, L_beta, L_alp, pow(L_m_H,2), pow(L_m_W,2), pow(L_m_W,2),sm);
...
void THDM::get_coupling_hhh_NLO(int h1,int h2,int h3,complex <double> &c) {
  ...
  double mh1 = get_hmass(h1);
  double mh2 = get_hmass(h2);
  double mh3 = get_hmass(h3);
  c = FCoup(L_MHL2,L_MHH2,L_MHA2,L_MHP2,L_M2,L_beta,L_alp,mh1*mh1,mh2*mh2,mh3*mh3,sm);
...
void THDM::store_param_phys() {
      get_param_phys(L_MHL,L_MHH, L_MHA, L_MHP,
                      L_sba, L_L6, L_L7,
                      L_M122, L_tb);
      L_MHL2 = L_MHL*L_MHL;
      L_MHH2 = L_MHH*L_MHH;
      ...
      L_MHP2 = L_MHP*L_MHP;
      L_beta = atan(L_tb);
      L_M2 = L_M122/(sin(L_beta)*cos(L_beta));
      L_alp = L_beta - asin(L_sba);
      Updated = true;
}
...
int GetNLOFuncSFF(int S1, int F1, int F2, int type, CFunc_NLO &func) {
    if (type == 1)
    {
        ...
        else if (S1 == 2)
        {
            ...
            {
                if (F1 == 11) {func = TypeI::CHHuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeI::CHHcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeI::CHHtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeI::CHHdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeI::CHHss; return NLOGOOD;}
                if (F1 == 23) {func = TypeI::CHHbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeI::CHHee; return NLOGOOD;}
                if (F1 == 32) {func = TypeI::CHHmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeI::CHHtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else if (S1 == 3)
        {
            ...
            {
                if (F1 == 11) {func = TypeI::CHAuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeI::CHAcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeI::CHAtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeI::CHAdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeI::CHAss; return NLOGOOD;}
                if (F1 == 23) {func = TypeI::CHAbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeI::CHAee; return NLOGOOD;}
                if (F1 == 32) {func = TypeI::CHAmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeI::CHAtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else
    ...
    else if (type == 2)
    {
        ...
        else if (S1 == 2)
        {
            ...
            {
                if (F1 == 11) {func = TypeII::CHHuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeII::CHHcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeII::CHHtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeII::CHHdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeII::CHHss; return NLOGOOD;}
                if (F1 == 23) {func = TypeII::CHHbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeII::CHHee; return NLOGOOD;}
                if (F1 == 32) {func = TypeII::CHHmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeII::CHHtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else if (S1 == 3)
        {
            ...
            {
                if (F1 == 11) {func = TypeII::CHAuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeII::CHAcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeII::CHAtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeII::CHAdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeII::CHAss; return NLOGOOD;}
                if (F1 == 23) {func = TypeII::CHAbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeII::CHAee; return NLOGOOD;}
                if (F1 == 32) {func = TypeII::CHAmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeII::CHAtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else
    ...
    else if (type == 4)
    {
        ...
        else if (S1 == 2)
        {
            ...
            {
                if (F1 == 11) {func = TypeLS::CHHuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeLS::CHHcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeLS::CHHtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeLS::CHHdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeLS::CHHss; return NLOGOOD;}
                if (F1 == 23) {func = TypeLS::CHHbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeLS::CHHee; return NLOGOOD;}
                if (F1 == 32) {func = TypeLS::CHHmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeLS::CHHtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else if (S1 == 3)
        {
            ...
            {
                if (F1 == 11) {func = TypeLS::CHAuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeLS::CHAcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeLS::CHAtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeLS::CHAdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeLS::CHAss; return NLOGOOD;}
                if (F1 == 23) {func = TypeLS::CHAbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeLS::CHAee; return NLOGOOD;}
                if (F1 == 32) {func = TypeLS::CHAmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeLS::CHAtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else
    ...
    else if (type == 3)
    {
        ...
        else if (S1 == 2)
        {
            ...
            {
                if (F1 == 11) {func = TypeFL::CHHuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeFL::CHHcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeFL::CHHtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeFL::CHHdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeFL::CHHss; return NLOGOOD;}
                if (F1 == 23) {func = TypeFL::CHHbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeFL::CHHee; return NLOGOOD;}
                if (F1 == 32) {func = TypeFL::CHHmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeFL::CHHtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else if (S1 == 3)
        {
            ...
            {
                if (F1 == 11) {func = TypeFL::CHAuu; return NLOGOOD;}
                if (F1 == 12) {func = TypeFL::CHAcc; return NLOGOOD;}
                if (F1 == 13) {func = TypeFL::CHAtt; return NLOGOOD;}
                if (F1 == 21) {func = TypeFL::CHAdd; return NLOGOOD;}
                if (F1 == 22) {func = TypeFL::CHAss; return NLOGOOD;}
                if (F1 == 23) {func = TypeFL::CHAbb; return NLOGOOD;}
                if (F1 == 31) {func = TypeFL::CHAee; return NLOGOOD;}
                if (F1 == 32) {func = TypeFL::CHAmm; return NLOGOOD;}
                if (F1 == 33) {func = TypeFL::CHAtata; return NLOGOOD;}
                return NOTIMPLEMENT;
            }
        }
        else
